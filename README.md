...
## 🚀 Quick Start - Local Independent Setup

**NEW**: OM1 now runs completely independent of OpenMind's API! Use your own API keys or run fully offline.

### Super Quick Setup (5 minutes)
```bash
git clone https://github.com/openmind/OM1.git
cd OM1
./setup_with_uv.sh  # Automated setup with UV
```
...
To get started with OM1's original setup, let's run the Spot agent. Spot uses your webcam to capture and label objects. These text captions are then sent to `OpenAI 4o`, which returns `movement`, `speech` and `face` action commands. These commands are displayed on WebSim along with basic timing and other debugging information.
